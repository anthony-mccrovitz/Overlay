...
def build_first_inning_stats(seasons: list[int], verbose: bool = True) -> dict:
    """
    Build cumulative first-inning statistics for pitchers and teams.

    Returns dict with:
        pitcher_1st_era: {pitcher_id: (runs, innings)} — first-inning ERA
        team_1st_scoring_rate: {team_id: (runs, games)} — how often team scores in 1st
    """
    pitcher_stats: dict[int, dict] = defaultdict(lambda: {"runs_allowed": 0, "games": 0})
    team_stats: dict[int, dict] = defaultdict(lambda: {"runs_scored": 0, "games": 0})

    for season in seasons:
        games = fetch_season_linescores(season, verbose=verbose)
        for g in games:
            ...
            if hp_id:
                pitcher_stats[hp_id]["runs_allowed"] += g["away_1st_inning_runs"]
                pitcher_stats[hp_id]["games"] += 1
            if ap_id:
                pitcher_stats[ap_id]["runs_allowed"] += g["home_1st_inning_runs"]
                pitcher_stats[ap_id]["games"] += 1

            if home_id:
                team_stats[home_id]["runs_scored"] += g["home_1st_inning_runs"]
                team_stats[home_id]["games"] += 1
            if away_id:
                team_stats[away_id]["runs_scored"] += g["away_1st_inning_runs"]
                team_stats[away_id]["games"] += 1

    return {
        "pitcher_1st_era": {
            pid: s["runs_allowed"] / max(s["games"], 1) * 9
            for pid, s in pitcher_stats.items()
        },
        "pitcher_1st_games": {
            pid: s["games"] for pid, s in pitcher_stats.items()
        },
        "team_1st_scoring_rate": {
            tid: s["runs_scored"] / max(s["games"], 1)
            for tid, s in team_stats.items()
        },
        "team_1st_games": {
            tid: s["games"] for tid, s in team_stats.items()
        },
    }
