...
def print_line_shop(results: list[LineShopResult], bankroll: float = 0) -> str:
    """Pretty-print line shopping results."""
    if not results:
        return "  No +EV lines found across sportsbooks.\n"

    lines = []
    lines.append(f"\n{'='*70}")
    lines.append("  LINE SHOPPING — Best Odds Across All Sportsbooks")
    lines.append(f"{'='*70}")

    for r in results:
        stars = ""
        if r.combined_edge >= 0.08:
            stars = " *** STRONG ***"
        elif r.combined_edge >= 0.05:
            stars = " ** GOOD **"

        lines.append(f"\n  {r.team} vs {r.opponent}{stars}")
        lines.append(f"    Model: {r.model_prob:.1%} | Market: {r.consensus_implied:.1%} | "
                     f"Best book: {r.best_implied:.1%}")
        lines.append(f"    Best odds: {r.best_odds:+d} at {r.best_sportsbook}")
        lines.append(f"    Edge: Model +{r.model_edge*100:.1f}% + "
                     f"Line +{r.line_edge*100:.1f}% = "
                     f"Total +{r.combined_edge*100:.1f}%")

        if len(r.all_books) > 1:
            worst = r.all_books[-1]
            lines.append(f"    Worst: {worst['odds']:+d} at {worst['sportsbook']} "
                        f"(line shopping saves {r.line_edge*100:.1f}%)")

    avg_combined = np.mean([r.combined_edge for r in results])
    avg_line = np.mean([r.line_edge for r in results])
    lines.append(f"\n  {'─'*40}")
    lines.append(f"  Total picks: {len(results)}")
    lines.append(f"  Avg combined edge: +{avg_combined*100:.1f}%")
    lines.append(f"  Avg line shopping value: +{avg_line*100:.1f}%")
    lines.append(f"{'='*70}\n")
